...
const PROXY = 'http://localhost:8000/api/v1/movie/';
...
logo.addEventListener('click', (e) => {
	if(main.hasChildNodes()) { removeAllChildren(main) }
	returnMovies(APILINK);
});


returnMovies(PROXY + 'movies');
function returnMovies(url) {
	...
	.then(function(data){
		...
		data.response.results.forEach(element => {
			...
			image.src = data.img_path + element.poster_path;
...
form.addEventListener("submit", (e)=>{
	...
	if(searchItem) {
		returnMovies(PROXY + "search/" + searchItem);
		search.value = "";
	}
})